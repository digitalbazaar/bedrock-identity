  ...
  async.auto({
    ...
    registerTriggers: ['createIndexes', function(callback) {
      mail.registerTrigger('getIdentity', function(event, callback) {
        api.get(null, event.details.identityId, function(err, identity) {
          if(!err) {
...
api.addPublicKey = function(actor, publicKey, privateKey, callback) {
  ...
  async.auto({
    ...
    add: ['checkPermission', function(callback) {
      _addPublicKey(publicKey, privateKey, callback);
    }]
  }, function(err, results) {
    callback(err, results.add);
  });
};
...
api.getPublicKey = function(publicKey, actor, callback) {
  ...
  async.auto({
    ...
    checkPermission: ['find', function(callback, results) {
      ...
      if(!record) {
        return callback(new BedrockError(
          'PublicKey not found.', 'NotFound', {key: publicKey}));
      }
      api.checkPermission(
 ...
 * @param callback(err, record) called once the operation completes.
 */
function _addPublicKey(publicKey, privateKey, callback) {
  ...
  async.auto({
    ...
    generateId: ['parse', function(callback) {
    ...
    insert: ['generateId', function(callback, results) {
      ...
      if(!('id' in publicKey)) {
        ...
        if(!('label' in publicKey)) {
          publicKey.label = util.format('Key %d', record.meta.lastPublicKeyId);
        }
      }
