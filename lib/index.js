    ...
    validate: function(callback) {
      if(identity.memberOf) {
        return callback(new BedrockError(
          'Creating an identity with a memberOf property is not acceptable. ' +
          'Please add membership via the brIdentity.update() call.',
          'NotAcceptable', {identity: identity}));
      }
      callback();
    },
    checkPermission: ['validate', function(callback) {
  ...
  async.auto({
    ...
    checkMemberOf: ['checkPermission', (callback, results) => {
      if(identity.memberOf) {
        ...
        const groups = identity.memberOf;
        return _checkMembershipUpdate(actor, identity, groups, callback);
      }
      callback();
    }],
...
function _checkMembershipUpdate(actor, identity, groups, callback) {
  // EXTERNAL: check to make sure IDENTITY_UPDATE_MEMBERSHIP permissions
  //           exists for actor
  // for each group in groups
  //  check to make sure group exists
  //  check to make sure group is a type: Group
  //  x check to make sure owner is actor OR
  //      actor has IDENTITY_UPDATE_MEMBERSHIP permission on resource??
  async.each(groups, (group, callback) => {
    async.auto({
      ...
      checkPermission: ['get', (callback, results) => {
        // FIXME: is this permission check correct?
        api.checkPermission(
