...
function _ensureSysResourceRoleValid(actor, identity, callback) {
  ...
  async.each(roleMappings, (roleMapping, callback) => {
    ...
    // exit early if there are no resources
    if(!roleMapping.resource) {
      return callback();
    }
    ...
    async.each(resources, (resource, callback) => {
      if(!_.isString(resource)) {
        return callback(new BedrockError(
          'Could not check permissions; provided resource is not a string',
          'InvalidResource', {
            sysResourceRole: roleMappings,
            resource: resource}));
      }
      async.auto({
        ...
        getRole: ['checkRoleDelegatePermission', callback => {
          ...
          brPermission.getRole(null, role, callback);
        }],
