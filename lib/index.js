...
api.getAll = function(actor, query, fields, options, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      _checkPermission(actor, PERMISSIONS.IDENTITY_ACCESS, callback);
    },
    function(callback) {
  ...
  */
  if('changes' in options) {
    ...
    if(options.changes.length === 0) {
      // no changes to make, return success
      return callback();
    }
...
api.setStatus = function(actor, identityId, status, callback) {
  async.waterfall([
    function(callback) {
      _checkPermission(actor, PERMISSIONS.IDENTITY_ADMIN, callback);
    },
    function(callback) {
 ...
 * @param callback(err) called once the operation completes.
 */
api.validateCapabilityDelegation = ({actor, resourceRoles}, callback) => {
  // FIXME: update `_ensureSysResourceRoleValid` to accept `resourceRoles`
  // rather than requiring the them to be passed via an `identity`
  const identity = {sysResourceRole: resourceRoles};
  _ensureSysResourceRoleValid(actor, identity, {}, callback);
};
 ...
 */
function _checkPermission(actor, permission, options, callback) {
  if(typeof options === 'function') {
    callback = options;
    options = {};
  }
...
function _updateIdentity(actor, identityId, options, callback) {
  async.auto({
    ...
    update: ['checkChanges', (callback, results) => {
      ...
      const identity = {};
      const pushFields = {};
      for(const operation of options.changes) {
        if(operation.op === 'set') {
          bedrock.util.extend(identity, operation.value);
          continue;
        }
        if(operation.op === 'add') {
          for(const key in operation.value) {
            ...
            if(field in pushFields) {
              pushFields[field].$each.push(...operation.value[key]);
            } else {
              pushFields[field] = {$each: [...operation.value[key]]};
            }
          }
          continue;
        }
...
function _checkChanges(actor, identity, changes, callback) {
  const cache = {};
  async.eachSeries(changes, (operation, callback) => {
    if(!operation.value || typeof operation.value !== 'object') {
      return callback(new BedrockError(
        `Could not update Identity. Change operation "${operation.op}" has ` +
        'no valid "value" field.', 'InvalidOperation',
        {operation: operation, httpStatusCode: 400}));
    }
    if(operation.op === 'set') {
    ...
    if(operation.op === 'add') {
      // TODO: compact `operation.value`? (would not work w/`sys` fields)
      delete operation.value['@context'];
      ...
      return async.each(Object.keys(operation.value), (key, callback) => {
        if(key === 'sysResourceRole') {
          return _ensureSysResourceRoleValid(
            actor, operation.value, {cache: cache}, callback);
        }
        if(key === 'memberOf') {
          return _ensureMembershipValid(actor, operation.value, callback);
        }
        /*
        ...
        callback(new BedrockError(
          ...
          'InvalidOperation', {operation: operation, httpStatusCode: 400}));
      }, err => callback(err));
    }
    // TODO: implement `remove`
    return callback(new BedrockError(
      `Could not update Identity. Invalid change operation "${operation.op}".`,
      'InvalidOperation', {operation: operation, httpStatusCode: 400}));
  }, err => callback(err));
}
...
function _checkDelegateCapability(actor, capabilities, callback) {
  ...
  async.each(capabilities, (capability, callback) =>
    async.auto({
      hasOwnCapability: callback => _checkPermission(
        actor, capability.permission,
        capability.resource ? {resource: capability.resource} : {},
        callback),
      canDelegate: callback => _checkPermission(
        actor, PERMISSIONS.IDENTITY_CAPABILITY_DELEGATE,
        capability.resource ? {resource: capability.resource} : {},
        callback)
    }, err => callback(err)),
    err => callback(err));
