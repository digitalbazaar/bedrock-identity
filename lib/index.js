...
api.insert = function(actor, identity, callback) {
  ...
  async.auto({
    ...
    insert: ['checkMemberOf', function(callback) {
...
api.update = function(actor, identity, options, callback) {
  ...
  */
  async.auto({
    ...
    checkMemberOf: ['checkPermission', callback => {
      if(identity.memberOf) {
        identity.memberOf = _cleanMemberOf(identity);
        return _ensureMembershipValid(actor, identity, callback);
      }
...
/**
 * Updates an Identity's ResourceRole.
 *
 * @param actor the Identity performing the action.
 * @param options the options to use:
 *   // FIXME: update doc
 * @param callback(err) called once the operation completes.
 */
api.updateRole = function(actor, options, callback) {
  if(!options || typeof options !== 'object') {
    throw new TypeError('options must be specified.');
  }
  if(typeof options.role !== 'string') {
    throw new TypeError('options.role must be a string.');
  }
  if(typeof options.identityId !== 'string') {
    throw new TypeError('options.identityId must be a string.');
  }
  if(typeof options.operation !== 'string') {
    throw new TypeError('options.operation must be a string.');
  }
  if(!['add', 'delete'].includes(options.operation)) {
    throw new TypeError('options.operation must be `add` or `delete`.');
  }
  if(typeof options.resourceId !== 'string') {
    throw new TypeError('options.resourceId must be a string.');
  }
  async.auto({
    checkPermission: callback => brPermission.checkPermission(
      actor, PERMISSIONS.IDENTITY_EDIT, {resource: options.identityId},
      callback),
    getResource: ['checkPermission', callback => api.get(
      actor, options.resourceId, err => callback(err))],
    getIdentity: ['getResource', callback => api.get(
      actor, options.identityId, (err, result) => callback(err, result))],
    checkRole: ['getIdentity', (callback, results) => {
      const roleIndex = _.findIndex(
        results.getIdentity.sysResourceRole, {sysRole: options.role});
      if(roleIndex === -1) {
        return callback(new BedrockError(
          'Invalid role specified.', 'NotFound', {role: options.role}));
      }
      const resource = results.getIdentity.sysResourceRole[roleIndex].resource;
      if(options.operation === 'add' && resource.includes(options.resourceId)) {
        // resource already set, no database update required
        return callback(null);
      }
      if(options.operation === 'delete' &&
        !resource.includes(options.resourceId)) {
        // resource already set, no database update required
        return callback(null);
      }
      const update = options.operation === 'add' ? {
        $push: {'identity.sysResourceRole.$.resource': options.resourceId}
      } : {
        $pull: {'identity.sysResourceRole.$.resource': options.resourceId}
      };
      database.collections.identity.update({
        id: database.hash(results.getIdentity.id),
        'identity.sysResourceRole': {$elemMatch: {sysRole: options.role}}
      }, update, database.writeOptions, err => callback(err));
    }]
  }, err => callback(err));
};
