...
bedrock.events.on(
  ...
  const {resource} = event.capability;
...
api.exists = brCallbackify(async ({actor, identity, meta}) => {
  ...
  if(meta.sysResourceRole.length > 0) {
    // FIXME: address identity.owner issue
    await api.validateCapabilityDelegation(
      {actor, resourceRoles: meta.sysResourceRole, owner: identity.owner});
  }
 ...
 * FIXME: @param owner an optional owner for the identity.
 *
 * @return a Promise that resolves once the operation completes.
 */
api.validateCapabilityDelegation = brCallbackify(async (
  {actor, resourceRoles, owner}) => {
  // TODO: fix `owner`?
  const options = {};
  if(owner) {
    options.owner = owner;
  }
  await _ensureSysResourceRoleValid({actor, resourceRoles, options});
});
...
async function _ensureSysResourceRoleValid(
  {actor, resourceRoles, options = {}}) {
  if(actor === null) {
