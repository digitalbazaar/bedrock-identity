...
api.updateRoles = brCallbackify(async (
  ...
  if(!isAdmin && remove.length > 0) {
    // if any roles are being removed, ensure actor has the capability
    await _checkPermission({
      actor,
    ...
    // TODO: another option would be to allow capabilities to be removed if
    // the actor is the original delegator, but this is tricky and not
  ...
  const resourceRoles = _.uniqWith(
    _addRoles(_removeRoles(meta.sysResourceRole, remove), add),
    _.isEqual);
...
api.validateCapabilityDelegation = brCallbackify(async (
  ...
  for(const capabilitySet of resourceRoles) {
    ...
    // TODO: unclear if we want to support "delegator" w/ocap changes
    /*if(actor && actor.id) {
...
function _removeRoles(resourceRoles, removalList) {
...
function _addRoles(resourceRoles, addList) {
