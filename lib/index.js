...
function _ensureSysResourceRoleValid(actor, identity, callback) {
  ...
  async.each(roleMappings, (roleMapping, callback) => {
    ...
    var resource = roleMapping.resource;
    ...
    async.auto({
      ...
      checkRoleDelegatePermission:
        ...
        if(results.getTargetResourceOwner) {
          resources.push(results.getTargetResourceOwner.owner);
        }
        api.checkPermission(
      ...
      checkAllRolePermissions: ['getRole', (callback, results) => {
        ...
        async.each(results.getRole.sysPermission, (permission, callback) => {
          console.log('checking', actor.id, permission, resource);
          api.checkPermission(
            actor, permission, {resource: resources}, callback);
        }, err => callback(err));
