...
api.exists = function(actor, id, options, callback) {
  ...
  options = options || {};
  async.series([
    callback => api.checkPermission(
      actor, PERMISSIONS.IDENTITY_ACCESS, {resource: id}, callback),
    callback => {
      var query = {
        id: database.hash(id)
      };
      query['identity.sysStatus'] = options.deleted ? 'deleted' : 'active';
      database.collections.identity.find(query, {limit: 1})
        .count((err, count) => callback(err, !!count));
    }
  ], (err, results) => callback(err, results[1]));
};
